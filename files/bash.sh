...
same_seconds_scheme() {
  case "$(roll_rand 0 1 2)" in 
    # Red base
    0)
      C1=$(calc216 "$1" $((5 - $1)) $((5 - $1)))
      C2=$(calc216 $((5 - $1)) "$1" "$1")
      ;;
    # Green base
    1)
      C1=$(calc216 $((5 - $1)) "$1" $((5 - $1)))
      C2=$(calc216 "$1" $((5 - $1)) "$1")
      ;;
    # Blue base
    2)
      C1=$(calc216 $((5 - $1)) $((5 - $1)) "$1")
      C2=$(calc216 "$1" "$1" $((5 - $1)))
      ;;
  esac
}
pick_scheme() {
  case "$(roll_rand 0 1 2)" in
    # Same control channel scheme
    0)
      same_ctrl_scheme "$1" "$2" "$3"
      ;;
    # Shift control channel scheme
    1)
      shift_ctrl_scheme  "$1" "$2" "$3"
      ;;
    # Inverted secondary channels from ctrl scheme
    2)
      same_seconds_scheme  "$1" "$2" "$3"
      ;;
  esac
}
...
pick_scheme "$ctrl_chan" "$chan1" "$chan2"
case "$(roll_rand 0 1)" in
    0) UC="$C1" HC="$C2" ;;
    1) UC="$C2" HC="$C1" ;;
esac
